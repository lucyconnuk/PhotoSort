...
def test_main(mocker):
    ...
    mocker.patch( 
        "photosort.get_command_line_args", 
        return_value = Namespace(
            source='C:\\Users\\lcbc\\Documents\\_Technology\\_GitRepos\\PhotoSort\\tests\\data',
            dest='C:\\Users\\lcbc\\Documents\\_Technology\\_GitRepos\\PhotoSort\\tests\\data'
        )
    )
...
def test_main_path_doesnt_exist(mocker):
    ...
    # Patch functions which will be called by the function under test
    mocker.patch( "photosort.get_command_line_args", return_value = Namespace( source='Z', dest='Y' ) )
    mocker.patch( "classes.AppLogger.logger.warning" )
